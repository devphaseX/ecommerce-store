...
import { ColourColumns } from './column';
...
const CellAction: React.FC<CellActionProps> = ({ data: { id: colourId } }) => {
  ...
  function onCopySaveToClipboard() {
    ...
    navigator.clipboard.writeText(colourId);
  ...
  function onBillboardEdit() {
    if (!activeClientEnv) return;
    router.push(`/${storeId}/colours/${colourId}`);
  }
